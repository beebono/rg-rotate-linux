...
int load_globals(char *data, struct mipi_dsi_device *dsi, struct generic_panel *ctx) {
    ...
    while (*data) {
        ...
        } else if (strcmp(param, "hs_rate") == 0) {
            int hs_rate;
            if (get_option(&val, &hs_rate) == 0) {
                dev_info(dev, "bad hs_rate %s\n", val);
            } else {
                dsi->hs_rate = hs_rate;
            }
        } else if (strcmp(param, "lp_rate") == 0) {
            int lp_rate;
            if (get_option(&val, &lp_rate) == 0) {
                dev_info(dev, "bad lp_rate %s\n", val);
            } else {
                dsi->lp_rate = lp_rate;
            }
        } else {
...
static int generic_panel_init_sequence(struct generic_panel *ctx)
{
    ...
    while (iseq) {
        ...
        } else if (ctx->use_dcs_write) {
            u8 dbuf[1 + iseq->len];

            dbuf[0] = iseq->dcs;
            memcpy(&dbuf[1], iseq->data, iseq->len);
            ret = mipi_dsi_dcs_write_buffer(dsi, dbuf, sizeof(dbuf));
            dev_dbg(dev, "iseq(dcs) %02x len=%d -> %d\n",
                    iseq->dcs, iseq->len, ret);
        } else {
            /*
             * Stock sends panel-private register writes as Generic Long Write
             * (type 0x29) with payload <reg, value>.  Sending them as DCS
             * Short Write 1 Param (type 0x15) routes them through the panel's
             * DCS command interpreter, which only knows standardized opcodes,
             * so every panel-private register write is silently dropped and
             * the panel stays in its post-reset default state.
             *
             * mipi_dsi_generic_write with a 2-byte buffer produces a Generic
             * Short Write 2 Params (type 0x23) carrying the same <reg, value>
             * payload — close to stock's 0x29 and routed through the same
             * generic-register-write path on the panel.
             */
            u8 gbuf[1 + iseq->len];
            gbuf[0] = iseq->dcs;
            memcpy(&gbuf[1], iseq->data, iseq->len);
            ret = mipi_dsi_generic_write(dsi, gbuf, 1 + iseq->len);
            dev_dbg(dev, "iseq(gen) %02x len=%d -> %d\n",
                    iseq->dcs, iseq->len, ret);
        }
        if (ret < 0) {
            dev_err(dev, "panel init command 0x%x failed: %d\n",
                    iseq->dcs, ret);
            return ret;
        }
        if (iseq->wait) {
...
static int generic_panel_prepare(struct drm_panel *panel)
{
    ...
    /* Stock sequence is physical high 50 ms, low 50 ms, high 120 ms. */
    ...
    /* Match the stock command table, including its padding byte and waits. */
    ret = mipi_dsi_dcs_write_buffer(dsi, sleep_out, sizeof(sleep_out));
    if (ret < 0) {
...

    {
        ...
        status_ret = mipi_dsi_dcs_get_power_mode(dsi, &power_mode);
        dev_info(ctx->dev, "DCS power mode: ret=%d value=%02x\n",
                 status_ret, power_mode);
        status_ret = mipi_dsi_dcs_read(dsi, MIPI_DCS_GET_DISPLAY_STATUS,
                                       display_status, sizeof(display_status));
        dev_info(ctx->dev,
                 "DCS display status: ret=%d value=%*ph\n",
                 status_ret, (int)sizeof(display_status), display_status);
        status_ret = mipi_dsi_dcs_read(dsi, MIPI_DCS_GET_DISPLAY_ID,
                                       id, sizeof(id));
...
static int generic_panel_probe(struct mipi_dsi_device *dsi)
{
    ...
    ctx->use_dcs_write = of_property_read_bool(dev->of_node, "use-dcs-write");
    ...
    dev_info(dev, "lanes %d, format %d, mode %lx use_dcs_write=%d\n",
             dsi->lanes, dsi->format, dsi->mode_flags, ctx->use_dcs_write);
