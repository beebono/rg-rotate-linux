...
- [ ] Audio — *stock HW:* in-PMIC codec `unisoc,sc2730-audio-codec` + `unisoc,vbc-v4-codec-sc2730`, VBC DMA engine `unisoc,sharkl5pro-vbc`, digital codec `unisoc,audio-codec-dig-agcp`; 2× external speaker amps `awinic,aw87xxx_pa` at i2c `0x58`/`0x59` (`spk-ext-pa-gpio` 0x09). Caveat: the routing/voice path leans on a large proprietary **AGDSP** stack (`audio_sipc`, `audio-mem-sharkl5pro`, `audio_dsp_{log,pcm,mem}`, `audio_pipe_{voice,effect,recordproc,bthal}`) over SIPC to a remote coprocessor — no mainline drivers (same shape as the DDR-DVFS stack).
