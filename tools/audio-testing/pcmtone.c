...
int main(int argc, char **argv) {
	...
	mask_set(pmask(&p, SNDRV_PCM_HW_PARAM_ACCESS), SNDRV_PCM_ACCESS_RW_INTERLEAVED);
	pmask(&p, SNDRV_PCM_HW_PARAM_ACCESS)->bits[0] = (1u<<SNDRV_PCM_ACCESS_RW_INTERLEAVED);
	...
	while (time(NULL) < end) {
		...
		if (ioctl(fd, SNDRV_PCM_IOCTL_WRITEI_FRAMES, &xfer)) {
			...
			perror("WRITEI"); printf("wrote %ld periods before err\n", w); return 4;
		}
		w++;
	}
